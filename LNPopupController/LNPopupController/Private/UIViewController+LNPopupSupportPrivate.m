...
- (  UIViewController *)_ln_common_childViewControllerForStatusBarHidden
{
...
- (  UIViewController *)_ln_childViewControllerForStatusBarHidden
{
	return [self _ln_common_childViewControllerForStatusBarHidden];
}

- (  UIViewController *)_ln_childViewControllerForStatusBarStyle
{
...
void _LNPopupSupportFixInsetsForViewController(UIViewController* controller, BOOL layout)
{
	...
	[controller.childViewControllers enumerateObjectsUsingBlock:^(LNObjectOfKind(UIViewController *)  obj, NSUInteger idx, BOOL *  stop) {
...
- (  UIView *)bottomDockingViewForPopup
{
	return self.tabBar;
}
...
- (void)hBWT:(NSInteger)t iE:(BOOL)e
{
	...
	[self.selectedViewController.transitionCoordinator animateAlongsideTransition:nil completion:^(id<UIViewControllerTransitionCoordinatorContext>  _  context)
	{
		[self._ln_popupController_nocreate _movePopupBarAndContentToBottomBarSuperview];
		[self._ln_popupController_nocreate.popupBar setHidden:self.tabBar.hidden];
		[self._ln_popupController_nocreate.popupContentView setHidden:self.tabBar.hidden];
...
- (void)_sTH:(BOOL)arg1 e:(unsigned int)arg2 d:(double)arg3;
{
	...
	[[self transitionCoordinator] animateAlongsideTransitionInView:self._ln_popupController_nocreate.popupBar.superview animation:^(id<UIViewControllerTransitionCoordinatorContext>  _  context) {
		//During the transition, animate the popup bar and content together with the toolbar transition.
		
		
		[self._ln_popupController_nocreate _setContentToState:self._ln_popupController_nocreate.popupControllerState];
	} completion:^(id<UIViewControllerTransitionCoordinatorContext>  _  context) {
